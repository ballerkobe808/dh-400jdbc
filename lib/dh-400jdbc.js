...
JDBCConn.prototype.isInitialized = function() {
  return (this._config && this._connectionPoolDataSource);
};
...
JDBCConn.prototype.prepareStatementWithParameterArray = function (sql, parameters, cb) {
  ...
  self._connectionPoolDataSource.getPooledConnection(function (err, pooledConnection) {
    // check if an error occurred.
    if (err) {
      return callback(err);
    }

    pooledConnection.getConnection(function (err, connection) {
      // check if an error occurred.
      if (err) {
        return callback(err);
      }
