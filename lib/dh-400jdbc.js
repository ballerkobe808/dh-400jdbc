...
var parameterTypes = require('./constants/parameter-types');
  ...
  self._connectionPool.getConnection(function (err, connection) {
    ...
    connection.prepareCall(sql, function (err, statement) {
      ...
      try {
        ...
        for (var i = 1; i <= parameters.length; i++) {
          if (_.isEqual(parameters[i-1].type, parameters.INPUT_PARAM)) {
            statement.setObjectSync(i, parameters[i - 1].value);
          }
          else if (_.isEqual(parameters[i-1].type, parameters.OUTPUT_PARAM)) {
            statement.registerOutParameter(i, parameters[i - 1].dataType);
          }
          else {
        ...
        // execute the update statement.
        statement.executeQuery(function (err, rs) {
          // check for an error.
          if (err) {
            return callback(err);
          }
