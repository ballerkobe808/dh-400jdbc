 ...
 * @param sql
 * @param rowCallback - The callback function to process a row.
 * @param finished - The finished callback funciton when all rows have been processed.
 */
exports.executeSqlString = function (sql, rowCallback, finished) {
  jdbc.initialize(config, function(err) {
    if (err) {
      return callback(err);
    }

    jdbc.execute(sql, rowCallback, finished);
  });
};
 ...
 * @param finished - The finished callback funciton when all rows have been processed.
 */
exports.executePreparedStatement = function (sql, parameters, rowCallback, finished) {
  jdbc.initialize(config, function(err) {
    if (err) {
      return callback(err);
    }

    jdbc.prepareStatementWithParameterArray(sql, parameters, rowCallback, finished);
  });
};
